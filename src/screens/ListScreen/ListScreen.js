...
const ListScreen = () => {
    ...
    return (
        <div data-testid='list-screen-container' className='list-screen-container flex'>

            <div className='flex' style={{ justifyContent: 'space-between' }}>
                <h1 style={{ maxWidth: 300 }}><Typography>Personagens exclusivos</Typography></h1>
                <input data-testid='input-search' className='input-search' type='text' placeholder='Digite sua busca' onChange={({ target }) => handleSearch(target.value)}/>
            </div>

            <div className='menu-cards'>
                {items?.map((item) => <Card width={300} height={412} item={item} onClick={handleCardClick} />)}
                <div data-testid='load-more-button' className='load-more-button' onClick={handleLoadMore}>

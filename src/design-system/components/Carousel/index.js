...
const Carousel = ({ items, onCallAction }) => {
    ...
    return items.length > 0 && selectedItem ? <div className='carousel-container' style={{ backgroundImage: selectedItem?.bg }}>
        ...
        <div className='carousel-content'>
            <div style={{ display: 'flex', height: "auto", minHeight: 730, left: 160, margin: 0, zIndex: 1 }}>
                <div className='col col-left'>
                    {items.map(item => <div key={item.id} className={`carousel-item ${selectedItem.id === item.id ? 'carousel-item-selected' : ''}`} style={{ backgroundImage: `url(${item.thumbnail.path}.${item.thumbnail.extension})` }} onClick={() => setSelectedItem(item)}></div>)}
                </div>
